...
HEAD = ["target", "source1", "source2", "source3", "ratio1", "ratio2", "ratio3", "std.error1", "std.error2",
        "std.error3", "tail", "Jackknife1", "Jackknife2", "Jackknife3"]
DIR = os.getcwd()
...
def process2(li):
    new_li = []
    new_li.extend(li[0:3] + [""] + li[5:7] + [""] + li[9:11] +
                  [""] + [li[12][:6], li[15][:6], li[16][:6], ""])
    return new_li
...
def process3(li):
    ...
    new_li.extend(li[0:4] + li[6:9] + li[11:14] +
                  [li[15][:6], li[18][:6], li[19][:6], li[20][:6]])
    return new_li
...
for line in file:
    ...
    if '-' in c3.value:
        ...
        flag = 0
    # 检查std. errors
    e1, e2, e3 = sheet_all.cell(row=sheet_row, column=8), sheet_all.cell(row=sheet_row, column=9), sheet_all.cell(
        row=sheet_row,
        column=10)
    if e1.value != "":
